...
public class Blocks implements Comparable<Blocks>
{
	...
	public static HashMap<String, Integer> calUniqMiners() throws FileNotFoundException, IOException
	{	
		...
		long numMiners = uniqMiners.size();
		HashMap<String, Integer> addressAndFrequency = new HashMap<String, Integer>();
		//System.out.println("Each unique Miner and its frequency:");
		for (int i = 0; i < uniqMiners.size(); ++i)
		{
			addressAndFrequency.put(uniqMiners.get(i), uniqMinersFreq.get(i));
			//System.out.println("Miner Address: " + uniqMiners.get(i) + "\nMiner Frequency: " + uniqMinersFreq.get(i) + "\n");
		}
		return addressAndFrequency;
	}
	...
	public static ArrayList<Integer> timeDiff(Blocks first, Blocks second) {
		...
		if ((first == null) || (second == null)) {
			...
			timeDiff.add(diffInHours);
			timeDiff.add(diffInMinutes);
			timeDiff.add(diffInSeconds);
